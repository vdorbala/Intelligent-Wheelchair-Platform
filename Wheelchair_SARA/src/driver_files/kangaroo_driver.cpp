...
kangaroo::kangaroo( ros::NodeHandle &_nh, ros::NodeHandle &_nh_priv ) :
	...
	nh( _nh ),
	nh_priv( _nh_priv ),
	encoder_lines_per_revolution(1440),
	hz(25),
	diameter_of_wheels(0.320)
...
void kangaroo::JointTrajCB(const trajectory_msgs::JointTrajectoryPtr &msg)
{
	...
	channel_1_speed = meters_to_encoder_lines(channel_1_speed);
	channel_2_speed = meters_to_encoder_lines(channel_2_speed);
...
void kangaroo::JointStateCB( const ros::WallTimerEvent &e )
{
	try
	{
		...
		msg->velocity[0] = encoder_lines_to_meters(msg->velocity[0]);
		msg->position[1] = encoder_lines_to_meters(msg->position[1]);
		msg->velocity[1] = encoder_lines_to_meters(msg->velocity[1]);
